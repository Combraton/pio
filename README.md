...
- **Codex:** your installed Codex with its own login and configured model. PIO never selects Codex's full-access sandbox or its unsandboxed shell-command surface. Starting a Codex thread in a project can add a trusted-project entry to your Codex configuration, as ordinary Codex use does; PIO discloses this and records the configuration before and after each run. Development tests run Codex only in throwaway fixture repositories.
