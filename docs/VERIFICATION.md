...
Five of the sixteen run **through the service**, `pio serve-claude` over the public Unix API, which is the only place journal, delivery and usage behaviour can be observed:
...
**Scope, stated plainly:** the other eleven cases drive the adapter and the fake directly, so they prove the adapter's decisions and the stream shapes. Host-loss without respawn is still **not** covered for this harness the way the Codex matrix covers it.
