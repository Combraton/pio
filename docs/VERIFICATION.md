...
`python3 scripts/opencode_host_matrix.py --out DIR --repetitions 3` is the **offline OpenCode matrix**, run in CI. Twenty-four cases, each three times, against that fake, and **never anywhere near the owner's running service**: the fake is a child of the matrix on stdio, and every case records the owner's `serve --service` process before and after and fails if it moved.
