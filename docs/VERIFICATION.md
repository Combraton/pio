...
`python3 scripts/claude_host_matrix.py --out DIR --repetitions 3` is the **offline Claude matrix**, run in CI. Ten cases, each three times, against the labeled fake: a turn that completes; the replay acknowledging delivery by exact echo; a permission decision denied and allowed; an out-of-fixture tool request declined with the tool use still recorded; a widening decision never sent; and, before any spawn, an unqualified executable, a missing credential route, a permission mode that is not the configured default, and surface drift — each refused. The drift case pins the fake's own surface as a baseline and then moves one help, so it proves drift rather than merely proving the fake is not Claude Code.

**Scope, stated plainly:** this matrix drives the adapter and the fake directly. It does not yet run through `pio serve-*` and the durable host the way the Codex matrix does, because the Claude service binding is not written. It therefore proves the adapter's decisions and the stream shapes, and proves nothing about journal, restart or reattach behaviour for this harness.
