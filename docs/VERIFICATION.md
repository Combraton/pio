...
`service_measures_where_the_harness_reports_usage` runs the same turn twice against a fake that reports usage only at the end and one that reports it on every message chunk, and asserts the census tells them apart: the update kinds counted, which of them carried usage, and the paths it was found at. It cannot say what OpenCode does — only a live run can — but it shows the measurement is capable of either answer, which a lookup at the one expected place was not.
