...
**The permission shape in these cases is specified, not measured.** The fake's `session/request_permission` comes from the ACP specification and has never been seen from OpenCode itself, so these cases prove what PIO does with the shape it expects, not what the harness sends. The first live request is the measurement. The fake asks only when a client announced itself with the handshake, because one that always asks cannot show a harness that does not.
