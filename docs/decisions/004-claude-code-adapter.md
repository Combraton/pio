...
- **A second user message sent mid-turn** is writable: stdin stays open and another `{"type":"user",…}` line can be sent. With `--replay-user-messages` the harness echoes it, and **that echo proves delivery and nothing more**. Whether the running turn incorporates it, queues it for the next turn, or drops it is **not observed**. M3 claims no steering, and **v0.1 does not steer a Claude run** (decided 2026-09-26, G4): an `execution.steer` to one is refused as `not_supported` with the reason `not_supported_mid_turn`, and nothing is written to the harness. Accepting it as queued would need a host that runs more than one turn and a measured queue. The `msg_lifecycle_v1` and `interrupt_cancel_queued_v1` capabilities are consistent with a queue existing; that is a hint, not a measurement.
