...
- **A watch position belongs to one stream.** Against another store it is dropped with a notice, not sent (`invalid_cursor` for ever). Gaps and epoch changes are delivered once by position, like events.
