...
L3 is L1's shape on Codex 0.157.0 and `gpt-5.6-terra` (`--plan L3`, lead `L3`, sequence `M4-lead-codex`, cap 685,000, stop 548,000 since the owner's decision of 2026-09-26 for its second attempt, Q9 "Child 90k"; it was 610,000, stop 488,000 under Q8 and 565,000, stop 452,000 earlier that day, and 400,000, stop 320,000 before). Its claim is **a steer under the lead's grant, delivered to a running Codex turn**. M2 showed Codex acknowledge the owner's steer; L3 shows it under the lead's authority. Behavior stays `not_observed`: delivery is not obedience.
...
- **The limits (owner, 2026-09-26, "More headroom", and for each child Q9, "Child 90k"; they were set on 2026-09-24).** Lead 150,000; child 90,000 (60,000 before Q9); a step at most 30,000, from the 22,000 to 24,500 per step M2 measured. Codex reports a step once its tool has finished, so a run can reach its ceiling plus two steps: the lead 210,000 and each child 150,000 (reviews of L3, rounds 1 and 2). The worst case is **510,000**, within the L3 sequence's 548,000 stop (cap 685,000; owner, 2026-09-26, Q9) after attempt 1's 36,126 (546,126), and, with Codex's 457,576 used, under the 968,000 Codex stop (cap 1,210,000): 967,576. Under Q8, earlier that day, it was 450,000 against 488,000 (cap 610,000) and 908,000 (cap 1,135,000). Before attempt 1 it was checked against 452,000 (cap 565,000) and 872,000 (cap 1,090,000), with Codex's 421,450 used: 871,450. Under the limits of 2026-09-24 it was 405,000, past every one of them, and the runner refused a live L3. That figure holds only with Codex's unmetered features off: sub-agents (with them on, 0.157.0's default, it would be 780,000 for multi-agent V2 and 1,050,000 for V1), memories, goals, standalone web search and image generation; the runner refuses unless each is off, by the owner's configuration or per launch under a recorded owner decision (rounds 3 and 4). The reservation is made immediately before the lead's submit. A run that ended by itself is charged Codex's reported total, the sum over its threads; one whose turn was cut short (the runner's stop, or an interrupt the host sent) that total plus a step in flight on each of its threads; the full rules are in the [evidence README](evidence/README.md). The runner stops a run by that total, by a step past 30,000, and by a sub-agent appearing.
...
- **The live relay (owner decision, 2026-09-25, item 3)** is `scripts/l3_desk_relay.py`. By itself it answers only:
  - `L3.alpha`'s and `L3.beta`'s command approvals, for exactly `sleep 30 && wc -l alpha.md` and `sleep 5 && wc -l beta.md` (bare, or as Codex names a command, `/bin/zsh -lc '…'`), each accepted once;
  - if Codex asks despite the pre-allowance, `L3`'s own `pio-lead` `start_run` and `read_run` approvals, each accepted once, with words that say the pre-allowance did not hold.

  Anything else goes to the owner live, with its 300 s deadline; unanswered, it lapses. The desk item now carries the command, the server and Codex's question, so the relay and the owner can see what is asked. `--relay` rehearses the relay against a desk that waits for its files, as live.
...
The screen's rule 1 now has crates behind it, and the dependency graph enforces it. **`crates/pio-client`** is the public API in Rust. **`crates/pio-client-cli`** is `pio client` and the caller ledger, a library whose one PIO dependency is pio-client; pio-cli, which links the service, only calls it. The screen's crate will be a third on the same footing. **`crates/pio-boundary`** checks each from a table: the graph with every feature on, an allow-list, the source files rustc's dep-info says it compiled (all inside the crate), and no symlink. Nothing about the boundary is a text search any more (proof and mutants, the review's B1 to B7 among them, in [verification](../../VERIFICATION.md#m4-t1--the-public-client-and-the-command-line-on-it)).
...
- **An exited run waits on nobody.** A pending action counts as needing approval, on the row and in the note, only while its run has not exited: the walk's rule, now the board's too. A waiting run cancelled on the OpenCode fake exits with its action still `pending` in the view (the host leaves it; tracked apart from T1), so the board reads it `uncertain` (exit unavailable, result absent), not "needs approval" (review of T1, round 2).
- **After a retention gap the walk says so, and is completed from the runs' views** (`walk::lost_to_retention`): the event that carried a request's deadline and options may be gone, but `actions[]` still lists it pending. Such a row is shown as waiting, marked *lost to retention*, with no deadline invented. The first cut dropped gap items and said "no approvals waiting" while two waited (review of T1).
- **The caller ledger's exit codes**: 3 when the service refused, 4 when the outcome is not known (`retry: after_reconcile`, such as an `internal_error` on submit) and the ledger holds it pending until a reconcile says.
- **A watch position belongs to one stream.** Against another store it is dropped with a notice, not sent (`invalid_cursor` for ever). Gaps and epoch changes are printed, once each by position, like events; a follower stopped on a page's last item moves past the page, so a page that ended in a gap is not read back as a longer gap.
