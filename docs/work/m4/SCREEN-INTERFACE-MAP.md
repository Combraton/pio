...
- **The limits (owner, 2026-09-24), and why L3 does not fit them yet.** Lead 125,000; child 50,000; a step at most 30,000, from the 22,000 to 24,500 per step M2 measured. Codex reports a step once its tool has finished, so a run can reach its ceiling plus two steps: the lead 185,000 and each child 110,000 (reviews of L3, rounds 1 and 2). The worst case is **405,000**, which is past the 320,000 stop, the 400,000 cap and, with Codex's 421,450 used, the 800,000 Codex stop, so **the runner refuses a live L3** until the owner re-sizes. That figure holds only with Codex's sub-agents off; with them on (0.157.0's default) it is 675,000 for multi-agent V2 and 945,000 for V1, and the runner refuses unless the owner's configuration or a recorded owner decision turns them off (round 3). The reservation is made immediately before the lead's submit. A run that ended by itself is charged Codex's reported total, the sum over its threads; one whose turn was cut short (the runner's stop, or an interrupt the host sent) that total plus a step in flight on each of its threads; the full rules are in the [evidence README](evidence/README.md). The runner stops a run by that total, by a step past 30,000, and by a sub-agent appearing.
