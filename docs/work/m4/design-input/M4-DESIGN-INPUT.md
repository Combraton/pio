...
1. **Use what Protocol v0.1.0 already has, and propose only what it lacks.** *(Corrected by the reviewer 2026-09-21 after reading the pinned schemas; the earlier wording asked for a proposal for all of it.)* Who started whom is already there: `execution.submit` carries `payload.origin {initiator, depth, call_budget}` and `execution.inspect` returns `origin`. PIO does not implement it yet; that is a PIO task, not a Protocol one. Budget pools and scoped grants also exist. What the pinned schemas do **not** carry is the author of a steering message: `steering_entry` and the event record are closed and have no author or grant field. Only that part needs a Protocol proposal, filed with demonstrating evidence. No local widening.
