...
Tracking: [issue #1](https://github.com/Combraton/pio/issues/1), [PR #2](https://github.com/Combraton/pio/pull/2). Mark ready after committing these dispositions. Merge, tag and publish remain separately authorized; branch M1 from main only after the merge.
...
| M1 — core and durable host | Cargo workspace, local protocol service, journal/outbox/deduplication, caller operation store, fenced launch barrier, host spool, scoped discovery and packaging skeleton | Named participant subset/controls and acceptance evidence below; macOS arm64 + Linux x86_64 CI build and conformance with pinned toolchain/lockfile. No real-adapter milestone claim |
