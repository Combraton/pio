...
3. **Done offline:** Codex qualification and configuration capture (`ea6bf17`), ADR 003 (proposed), and the adapter connected through the durable host with a 13-case offline matrix (`5095f06`). Live-run plan posted on #5; **waiting for the owner's go**. Remaining for this step: then connect app-server through the durable host without relaxing existing fences or permissions.
