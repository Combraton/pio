...
- **MCP tool approvals (static, from source).** Read at `rust-v0.155.1` (`be2951e…`) and `rust-v0.157.0` (`00c972e…`), in `codex-rs/core/src/mcp_tool_call.rs`, `codex-rs/codex-mcp/src/mcp/mod.rs` and `codex-rs/features/src/lib.rs`. Each function that decides and asks is identical at the two tags:
