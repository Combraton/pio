...
**Correction, 2026-09-23: 86,863 charged.** OpenCode's turn usage is its **last model step's**. R2, R3 and R5 each made a tool call and so took two steps, and only the second was recorded. Their first steps were read back, read-only, from the owner's OpenCode store, from rows for these PIO-started sessions (exactly what was read is in the M4 map): 8,135, 8,238 and 8,270, or 24,643 together. They are charged as separate ledger lines. The figures above are the receipts as recorded. See the [M4 map](../m4/SCREEN-INTERFACE-MAP.md), L1, review 44.
