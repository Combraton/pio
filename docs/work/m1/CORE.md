...
Final self-review added regression coverage for required/optional feature overlap, duplicate-profile precedence on renegotiation, and non-JSON whitespace frames. Vertical tab and form feed produce a fatal `parse_error`; only space, tab and carriage return qualify as an ignored blank frame. The local Cargo suite has nine tests. A read-only fixture audit informed coverage mapping; a broad MiniMax M3 code review and one narrowed retry were stopped without final reports, so no external code-review verdict is claimed.
