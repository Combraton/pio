...
## Core participant — 2026-09-16

Base remains `900bc03`; predecessor/correction head `38ca384`. The [Core checkpoint](CORE.md) replaces empty participant claims with independently implemented Core/core-test/grants/events/capabilities. Local pinned-runner results are 164 pass, 115 unsupported and 1 skipped with no failures; per-directory counts are in the report. Stream contributes 24/24 passes. Public effects remain unclaimed until their pinned Execution fixtures run. Seven Cargo unit tests include atomic protocol rollback, response loss and internal effect wait semantics. The matrix now records 54 attempts with named defense and wrong-reason controls. Current head, CI and clean-clone evidence are recorded in [STATE](../STATE.md).
...
## Owner milestone acceptance — 2026-09-16

**M1 accepted at `2048e84b0e77b258444f8f30333f4b76584cdccc`**, base `900bc03cfb0c898faa14f5e8b68afe4ac7ff26fe`, branch `codex/m1-core-host`. The owner reproduced the full documented sequence from a clean clone, including real launchd start/stop on a workstation, confirmed both CI platforms, and independently mutated ordering: the mutant now fails on `dispatch_intent_must_precede_invocation_intent`. This owner disposition supersedes the milestone-pending status in earlier dated records; their evidence, mapping and limits remain unchanged.

The [acceptance corrections report](ACCEPTANCE-CORRECTIONS.md), [receipt index](evidence/acceptance-corrections.json) and [accepted-head CI](https://github.com/Combraton/pio/actions/runs/35097214909) remain the evidence references: 20 tests, runner 206 pass / 73 unsupported / 1 skipped plus two separate supplemental passes, public matrix 66 attempts and diagnostic matrix 54, caller recovery and both native user-job lifecycle checks. No real harness/native authentication/end-to-end journey is proven; all six journeys remain `not_evaluated`. Every stated coverage and implementation limit is retained.

This final PR commit is documentation only: acceptance status and the [M2 task packet](../m2/TASK.md)/[fresh-session kickoff](../m2/KICKOFF.md). No M2 implementation begins here. [Owner acceptance is recorded on issue #3](https://github.com/Combraton/pio/issues/3#issuecomment-5699087663); **only after the separately authorized merge**, close #3 and open the M2 issue linking the packet. The merged-main base must be resolved and recorded then, not invented before merge. Merge, tag and publication remain separate authorizations. Historical M1 documents and evidence directories are preserved.
