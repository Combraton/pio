...
- **Next:** qualify the selected Codex executable. This is the Node wrapper `codex.js` (npm `@openai/codex` 0.146.0), which launches the platform package's native `codex`; bind the wrapper, Node runtime and native binary paths, hashes and version, plus per-file canonical schema identity. Build before/after capture of the user's Codex configuration. Record the adapter design (brief-byte delivery by digest, host-owned app-server, approvals, token accounting) in a decision record, then connect app-server through the durable host. **Before the first live run, post the fixture repository path, live task text, expected token cost and run plan against the 1,000,000 bound on issue #5 and wait for the owner's go.** Live evidence goes in a fresh directory under `docs/work/m2/`. Spend ledger remains at zero.
