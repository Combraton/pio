...
- **M3/M3b current counts:** **100 Cargo tests + 1 ignored probe**; Claude matrix **26 × 3 = 78**, fourteen through `serve-claude`; OpenCode matrix **24 × 3 = 72**, eleven through `serve-opencode`, plus the OpenCode runner self-test and the private-path gate; Codex matrix, public matrix and diagnostic matrix unchanged at 51, 72 and 54, which is the proof the shared work changed nothing else.
...
- **Next: M4, the terminal screens**, against the design the owner accepted on 2026-09-21, copied to [`m4/design-input/`](m4/design-input/M4-DESIGN-INPUT.md). **Step 1 is posted and waiting for the reviewer's read**: the [screen-to-interface map](m4/SCREEN-INTERFACE-MAP.md) on [issue #12](https://github.com/Combraton/pio/issues/12), naming the public operation behind every element of every screen. Seven gaps; six are one shape — *the host records it and nothing public can reach it*. **G1** (no query lists the executions a caller can see) and **G2** (no cross-run pending-request query with deadlines) are the only two needing new query surface. One expected gap is **not** a gap: `core.events.read` already takes a cursor. Budget pools and scoped grants are implemented; **origin with initiator, depth and call budget is not**, and that is a Protocol gap rather than a PIO one.
