...
| MiniMax via OpenCode | 300,000,000 | 240,000,000 | **62,220 charged**, 50,220 observed | 7 |
...
- **M3b is merged** as `a22c98c` (PR #11). Seven live MiniMax turns on OpenCode 2.0.11, **62,220 charged / 50,220 observed** of 300,000,000. Two partial acceptances (J1, J4), J3 and J5 `not_evaluated` for this harness, seven open obligations: the [M3b packet](m3b/ACCEPTANCE.md). The permission finding is the one to carry forward: **this harness asks about some operations and not others** — a read outside the workspace prompts and PIO declines it; a shell command inside it does not prompt and simply runs, under either posture.
- **Next: M4, the terminal screens**, against the design the owner accepted on 2026-09-21, copied to [`m4/design-input/`](m4/design-input/M4-DESIGN-INPUT.md). **Step 1 is posted and waiting for the reviewer's read**: the [screen-to-interface map](m4/SCREEN-INTERFACE-MAP.md) on [issue #12](https://github.com/Combraton/pio/issues/12), naming the public operation behind every element of every screen. Revised after the reviewer's read: **the inspect result is `unevaluatedProperties: false`**, so nothing is promoted into the view — every extra field travels by a Protocol **event payload** (which is open by schema) or a namespaced PIO profile, and **no new name is added under `execution.*` or `core.*`**; conformance stays at **206**. **G1 and G6 try the Protocol-native fold first**: `core.events.read {from: start, kinds: [execution]}` enumerates every visible subject, subscribe gives the changes, and inspect is called only for subjects that moved. `origin {initiator, depth, call_budget}` **is** in the pinned schema and is a **PIO** gap, not a Protocol one; the only real Protocol gap is that `steering_entry` and the event record are closed and carry no author.
...
- **No further Claude or MiniMax live runs are planned.** Hermes stays deferred: test scope only, not gating v0.1.
