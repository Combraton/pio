...
- **M3/M3b current counts** at `66c6776`: **97 Cargo tests + 1 ignored probe**; Claude matrix **17 × 3 = 51**, six through `serve-claude`; OpenCode matrix **12 × 3 = 36**, two through `serve-opencode`; Codex matrix, public matrix and diagnostic matrix unchanged at 51, 72 and 54, which is the proof the shared-lifecycle extraction changed nothing else.
...
- **Next: both live plans are posted and waiting for the owner's go.** The [Claude plan](https://github.com/Combraton/pio/issues/7) and the [MiniMax plan](https://github.com/Combraton/pio/issues/10) are the last gate; **no live run of any harness happens before its plan is reviewed**. A fresh session starts from the [kickoff prompt](m3/KICKOFF.md), then this file, then the [task packet](m3/TASK.md).
