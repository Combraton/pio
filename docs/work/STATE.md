...
- **Next:** request independent review of the capacity-bound checkpoint on the draft PR. Then qualify the selected Codex executable, version, hash and per-file canonical schema identity, and build before/after capture of the user's Codex configuration, before any live run. Spend ledger remains at zero.
