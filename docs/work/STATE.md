...
- **M3b is assembled.** Seven live MiniMax turns on OpenCode 2.0.11, 62,220 charged and 50,220 observed of 300,000,000. Two partial acceptances (J1, J4), J3 and J5 `not_evaluated` for this harness, seven open obligations: the [M3b packet](m3b/ACCEPTANCE.md).
- **M3 is accepted and merged.** PR #9 merged at `7791f56` (merge commit `a76758b`) as four partial acceptances with seven named obligations. The Claude sequence is closed and **no further Claude runs happen**.
- **Next: M3b, MiniMax R1 to R5 straight through** (owner decision, 2026-09-21), on a fresh branch and draft pull request from that main, under the plan on [issue #10](https://github.com/Combraton/pio/issues/10): `minimax-coding-plan/MiniMax-M2.7-highspeed` on every run, each run rehearsed as a dry run first, a clean committed head, receipts posted as they land. **The stop rules stay in force:** an unplanned missing usage report, a session whose provider or model is not the requested one, any change to the owner's OpenCode service, or 80 per cent of the cap. **R1 is the usage measurement** — which session update kinds carry usage, and whether a cancelled turn keeps it; if it does not, the Claude allowance rule applies with its own evidence-based figure. **Hermes is deferred.** After R5: the M3b packet as test-scope evidence, then the M4 preparation at zero tokens — incremental changed-key commits, the per-message usage reader for Claude Code, and an M4 task packet with a kickoff prompt. **The terminal UI does not start until the reviewer's mockups arrive.** A fresh session starts from the [kickoff prompt](m3/KICKOFF.md), then this file, then the [task packet](m3/TASK.md) and the [M3 packet](m3/ACCEPTANCE.md).
